...
pattern = re.compile(r"^chatgpt小助手\s*(.*)$")  # 匹配以chatgpt小助手开头的字符串，并获取后面的内容
...
async def handle_function(event: Event):
    ...
    if message:
        ...
        if not match:
            # 如果匹配失败
            await chatgpt.finish("命令格式错误，请输入 chatgpt小助手 + 需要查询的内容")
            return
        # 获取正则表达式中定义的分组所匹配的内容
        query = match.group(1)  
...
def requestApi(msg):
    ...
    response = requests.get('http://127.0.0.1:8000/chatgpt/?msg='+msg)
